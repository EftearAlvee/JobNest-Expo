...
const RecruiterDashboardScreen = () => {
  ...
  const greeting = (() => {
    ...
    if (hours >= 17 || hours < 5) return 'Good Evening';
    return 'Good Morning';
  })();

  const recentApplications = [
    { id: 1, name: 'Alice Johnson', role: 'Frontend Developer', status: 'Pending' },
    { id: 2, name: 'David Smith', role: 'UI/UX Designer', status: 'Shortlisted' },
    ...
    <SafeAreaView className="flex-1 bg-gray-50">
      <ScrollView className="px-5 pt-6" showsVerticalScrollIndicator={false}>
        <View className="flex-row justify-between items-center mb-6">
          ...
          <TouchableOpacity
            className="border-2 border-blue-100 rounded-full p-1"
            onPress={() => router.push('/recruiter/profile')}
          >
            <Image source={require('../../assets/images/alvee2.jpg')} className="w-12 h-12 rounded-full" />
          </TouchableOpacity>
        </View>
        ...
        <View className="flex-row flex-wrap justify-between mb-8">
          {[
            { label: 'Total Jobs', value: jobStats.totalPosts, color: 'text-blue-600' },
            { label: 'Applications', value: jobStats.totalApplications, color: 'text-green-600' },
            { label: 'Shortlisted', value: jobStats.shortlisted, color: 'text-purple-600' },
            { label: 'Interviews', value: jobStats.interviews, color: 'text-yellow-600' },
          ].map((stat, idx) => (
            <View
              key={idx}
              className="bg-white p-4 rounded-xl w-[48%] mb-4 shadow-sm border border-gray-200"
            >
              <Text className={`text-2xl font-bold ${stat.color} text-center`}>{stat.value}</Text>
              <Text className="text-center text-gray-600 mt-1">{stat.label}</Text>
            </View>
          ))}
        </View>
        ...
        <View className="mb-6">
          <View className="flex-row justify-between items-center mb-4">
            ...
            <TouchableOpacity onPress={() => router.push('/recruiter/jobs')}>
            ...
            {activeJobs.length > 0 ? (
              activeJobs.map((job) => (
                <TouchableOpacity
                  ...
                  onPress={() => router.push(`/recruiter/job-details/${job.id}`)}
                >
              ...
              <View className="bg-white p-5 rounded-xl shadow-sm shadow-gray-300 border border-gray-100 items-center">
                ...
                <TouchableOpacity className="mt-2" onPress={() => router.push('/recruiter/post-job')}>
